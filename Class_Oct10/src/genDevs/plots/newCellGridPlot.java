 ...
 */
public class newCellGridPlot extends JobQueue
{
    /**
     * The cell-grid-view on to which to plot.
     */
    protected newCellGridView cellGridView;
     ...
     */
    public newCellGridPlot(String name, double jobDueDelay,
        String xLabel, double xRange_, String yLabel, double yRange_)
    {
        ...
        cellGridView = new newCellGridView(name, xLabel, yLabel, xRange, yRange);
        // cellGridView.run();
     ...
     */
    public newCellGridPlot(String name, double jobDueDelay,
        newCellGridView cellGridView_, double zRange_)
    {
        super(name, jobDueDelay);
     ...
     * A convenience constructor.
     */
    public newCellGridPlot(String name, double delay,
     ...
     * A convenience constructor.
     */
    public newCellGridPlot(String name, double delay, double yRange)
    {
        ...
        for (int i = 0; i < message.getLength(); i++) {
            ...
            // else, if this content is on the draw-cell-to-scale port
              ...
              //  DrawCellEntity entity = DrawCellEntity.toObject(inEntity);
                ...
                // remember the operation that must be performed to dim what
                // was drawn just above
                cellGridView.drawCellToScale(entity.i, entity.j,
                          entity.color);
                entity.setName("drawCellToScale");
            }

            else if (messageOnPort(message, "drawAgentCellToScale", i)) {
                // add this content to the jobs that have arrived
                entity inEntity = message.getValOnPort("drawAgentCellToScale", i);
                DrawCellEntity entity = (DrawCellEntity)inEntity;
              //  DrawCellEntity entity = DrawCellEntity.toObject(inEntity);
                arrived.put(dueTime, entity);
                ...
                // tell the cell-grid-view to draw-to-scale the cell
                // specified by the job
                cellGridView.drawAgentCellToScale(entity.i, entity.j, entity.color);

                // remember the operation that must be performed to dim what
                // was drawn just above
                entity.setName("drawAgentCellToScale");
            }
     ...
     * See member variable accessed.
     */
    public newCellGridView getCellGridView() {return cellGridView;}
    public void setCellGridView(newCellGridView view)
    {
        cellGridView = view;
        configureCellGridView();
    }
